...
if __name__ == '__main__':
    # folder1 = "DoubleDQL_Soft-Update (0.005)"
    folder2 = "DDQL_2ndEncoding (Min-Max norm.)"
    # folder3 = "DDQL_2ndEncoding (NO norm.)"
    # folder4 = ""
    ...
    for idx in range(3):
        ...
        planner = Planner(env_norm, pathtomodel=f"{MODELS_FOLDER}{PROBLEM}/{folder2}/{PROBLEM}-{idx}.h5", reduceactionspace=REDUCE_ACTION_SPACE)
    ...
    # env.get_previous_plans(REDUCE_ACTION_SPACE)

    # for idx in range(3):
    #     agent = DDQNAgent(env_norm)
    #
    #     print(f"------------------------------- {idx} -------------------------------")
    #
    #     exp_results = deep_q_learning_alg(env_norm, agent, idx, folder3)
    #     exp_results.save_data(folder3, idx)
    #
    #     planner = Planner(env_norm, pathtomodel=f"{MODELS_FOLDER}{PROBLEM}/{folder3}/{PROBLEM}-{idx}.h5", reduceactionspace=REDUCE_ACTION_SPACE)
    #     solution, score, finished = get_plan(env_norm, agent, reduceactionspace=REDUCE_ACTION_SPACE)
